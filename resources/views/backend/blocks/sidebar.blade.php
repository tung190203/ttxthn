                        ...
                        @if (!empty($backend_items['items']))
                            @can($backend_parent_key)
                                <li class="nav-item has-treeview @if ($selectedMainMenu == $backend_parent_key) menu-open @endif">
                                    <a href="#!" class="nav-link @if ($selectedMainMenu == $backend_parent_key) active @endif">
                                        <i class="nav-icon {{ $backend_items['icon'] }}"></i>
                                        <p>
                                            {{ $backend_items['title'] }}
                                            @if (isset($pendingCounts[$backend_parent_key]) && $pendingCounts[$backend_parent_key] > 0)
                                                <span class="badge badge-danger right approval-count"
                                                    style="right: 0.5rem;">{{ $pendingCounts[$backend_parent_key] }}</span>
                                            @endif
                                    ...
                                    <ul class="nav nav-treeview">
                                        @foreach ($backend_items['items'] as $backend_key => $backend_item)
                                            @php
                                                $permissionKey =
                                                    $backend_parent_key === 'user'
                                                        ? $backend_key
                                                        : $backend_parent_key . '/' . $backend_key;
                                            @endphp
                                            @can($permissionKey)
                                                <li class="nav-item">
                                                    <a href="{{ route($backend_item['route']) }}"
                                                        ...
                                                        <p>
                                                            {{ $backend_item['title'] }}
                                                            @if ($backend_key == 'approval' && isset($pendingCounts['approval']) && $pendingCounts['approval'] > 0)
                                                                <span class="badge badge-danger right approval-count">
                                                                    {{ $pendingCounts['approval'] }}
                                                                </span>
                                                            @endif
                                                        </p>
                                                    </a>
                                ...
                                <li class="nav-item">
                                    <a href="{{ route($backend_items['route']) }}"
                                        ...
                                        <p>
                                            {{ $backend_items['title'] }}
                                            @if (isset($pendingCounts[$backend_parent_key]) && $pendingCounts[$backend_parent_key] > 0)
                                                <span class="badge badge-danger right approval-count">{{ $pendingCounts[$backend_parent_key] }}</span>
                                            @endif
                                        </p>
                                    </a>
                                </li>
