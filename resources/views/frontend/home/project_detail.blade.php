...
<script>
    ...
    $(document).ready(function() {
        $('.project-nav__list a').click(function(e) {
            e.preventDefault(); // Ngăn browser scroll mặc định
            ...
            if (targetEl.length) {
                const offset = 150; // Khoảng cách tính từ top
                const scrollTop = targetEl.offset().top - offset;

                $('html, body').animate({
                    scrollTop: scrollTop
                }, 500); // 500ms là tốc độ scroll
            }
        });
    });
</script>
